...
-export([loop2DGossipNodes/3, loopGetExistingNeighbor2D/2, spawn2DGossipNodes/1, spawn2DPushSumNodes/1,gossipNodes2D/1]).
-export([getRandomNeighbor2D/1, loop2DPushSumNodes/4, loopGetExistingNeighborPushSum2D/4,pushSumNodes2D/4]).
...
goGossip(NumNodes) ->
  ...
  Pid = whereis(Name),
  Pid ! {rumors, [RumorStarter_X, RumorStarter_Y]}.
...
loop2DGossipNodes(X, Y, R) ->
  Pid = spawn(topology2D, gossipNodes2D, [0]),
...
goPushSum(NumNodes) ->
  ...
  Pid = whereis(Name),
  Pid ! {rumors, 0, 1, [RumorStarter_X, RumorStarter_Y]}.
...
loop2DPushSumNodes(X, Y, R, NumNodes) ->
  Pid = spawn(topology2D, pushSumNodes2D, [NumNodes, 1, [X, Y], 0]),
...
loopGetExistingNeighborPushSum2D(X, Y, S, W) ->
  NeighborCoordinate = getRandomNeighbor2D([X, Y]),
  Neighbor_X = lists:nth(1, NeighborCoordinate),
  Neighbor_Y = lists:nth(2, NeighborCoordinate),
